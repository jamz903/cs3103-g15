...
class ConnectionManager:
    ...
    async def connect(self, websocket: WebSocket, client_id: str):
        ...
        async with self.lock:
            # Case 1: No current speaker
            if self.current_speaker is None:
                self.current_speaker = websocket
                self.active_connections.append(websocket)
                return True
                
            # Case 2: Already connected as speaker
            if any(ws.client_id == client_id for ws in self.active_connections):
                return True  # Keep connection open, let them continue
                
            # Case 3: Different speaker active
